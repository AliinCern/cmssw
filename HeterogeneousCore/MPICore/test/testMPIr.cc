...
const std::pair<float, float> nonBlockScatter(MPIData& mpiInput) {
  ...
  if (mpiInput.rank)  //Only for Workers
  {
    ...
    cudaMalloc((void**)&d_vect1, mpiInput.vectorWorkers1.size());//allocate memory space for vector in the global memory of the Device.
    cudaMalloc((void**)&d_vect2, mpiInput.vectorWorkers2.size());
    cudaMalloc((void**)&d_vect3Gpu, mpiInput.vectorWorkers1.size());

    cudaMemcpy(d_vect1, mpiInput.vectorWorkers1.data(), mpiInput.vectorWorkers1.size(), cudaMemcpyHostToDevice);//copy random vector from host to device.
    cudaMemcpy(d_vect2, mpiInput.vectorWorkers2.data(), mpiInput.vectorWorkers1.size(), cudaMemcpyHostToDevice);

    int threads = 512; //arbitrary number.
    int blocks = (size + threads - 1) / threads; //get ceiling number of blocks.
    blocks = std::min(blocks, 8); // Number 8 is least number can be got from lowest Nevedia GPUs.
    
    addVectorsGpu<<<blocks,threads>>> (d_vect1, d_vect2, d_vect3Gpu, n); //call device function to add two vectors and save into vect3Gpu.
